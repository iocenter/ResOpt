...
bool GprsSimulator::generateMainInputFile(Model *m)
{
    ...
    {
        qWarning("Could not connect to output file: %s", main_file.fileName().toLatin1().constData());
        exit(1);
    }
    ...
    *out_main << "INCLUDE " << m->reservoir()->file().toLatin1().constData() << "\n\n";
...
bool GprsSimulator::generateWellInputFile(Model *m)
{
    ...
    {
        qWarning("Could not connect to output file: %s", well_file.fileName().toLatin1().constData());
        exit(1);
    }
    ...
    for(int i = 0; i < m->numberOfWells(); i++)
    {
        ...
        *out_well << "#  WELL_NAME\tGROUP\tRES_NAME\tTYPE\tSTATUS" << "\n";
        ...
        // wellspecs
        *out_well << "   "
                  << w->name().toLatin1().constData() << "\t"
                  << w->group().toLatin1().constData() << "\t"
                  << m->reservoir()->name().toLatin1().constData() << "\t"
                  << w->typeAsString().toLatin1().constData() << "\t"
                  << "OPEN" << "\n\n";
...
bool GprsSimulator::generateControlInputFile(Model *m)
{
    ...
    {
        qWarning("Could not connect to output file: %s", ctrl_file.fileName().toLatin1().constData());
        exit(1);
    }
...
bool GprsSimulator::readWellOutput(Well *w, QString file_name)
{
    ...
    {
        qWarning("Could not open WELL output file: %s", file_name.toLatin1().constData());
        exit(1);
    }
